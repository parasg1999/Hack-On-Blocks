    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-x-4 gap-y-4 mt-8 w-full mx-auto mb-24 mt-24">
        {numbers.map((s, i) => (
          <div
            key={i}
            onClick={() => window.open(`https://${s.link}`, "_blank")}
          >
            <div
              className={
                "flex flex-col font-satoshi gap-y-3 px-2 py-4 md:px-4 md:py-8 items-center justify-between h-full bg-white rounded-lg md:rounded-2xl relative"
              }
            >
              <Image
                src={s.img}
              ...
              />
              <div className="absolute top-0 w-full min-h-[64px] md:min-h-[128px] bg-black z-20 mt-4 md:mt-8 transparent-gradient"></div>
              <a
                style={{
                ...
                className="uppercase text-[#FCFDFE] px-4 md:px-8 py-2 font-semibold tracking-widest min-w-[80%] flex flex-row justify-center text-xs md:text-lg mt-[-0.75rem]"
              >
                {s.heading}
              </a>
              <div className="px-2 md:px-2 gap-x-2 flex flex-row font-semibold text-sm md:text-lg self-start align-center">
